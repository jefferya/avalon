...
module MasterFileManagementJobs
  class Move < ActiveJob::Base
    ...
    def perform(id, newpath)
      Rails.logger.info "Moving masterfile to #{newpath}"
      Rails.logger.info "zzzzz 8b Moving masterfile to #{newpath}"
      ...
      if old_locator.exists?
        ...
        Rails.logger.info "zzzzz 8c Moving masterfile from #{oldpath} to #{newpath}"
      else
        Rails.logger.error "MasterFile #{oldpath} does not exist"
        Rails.logger.info "zzzzz 8d #{oldpath} does not exist"
      end
      Rails.logger.info "zzzzz 8e Moving masterfile to #{masterfile.inspect}"
    end
  end
